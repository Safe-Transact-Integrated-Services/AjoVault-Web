...
const Modal = ({
  ...
  // Determine variant based on props if not explicitly provided
  const effectiveVariant = children ? 'content' : variant;
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">
            {title || (effectiveVariant === 'otp' ? 'Verify OTP' : 'Modal')}
          </DialogTitle>
          {description && <DialogDescription>{description}</DialogDescription>}
          {effectiveVariant === 'otp' && !description && (
            <DialogDescription>
              Enter the code sent to you to verify your identity.
            </DialogDescription>
          )}
        </DialogHeader>

        {effectiveVariant === 'otp' ? (
          <div className="flex flex-col items-center space-y-6 py-4">
              ...
              <InputOTPGroup className="gap-2">
                {Array.from({ length }).map((_, i) => (
                  <InputOTPSlot
                    key={i}
                    index={i}
                    className={`h-12 w-12 rounded-lg border-2 text-lg font-bold transition-all ${
                      error ? 'border-destructive text-destructive' : 'border-border focus:border-[#102A56]'
                    }`}
                  />
                ))}
              </InputOTPGroup>
            ...
            <div className="flex w-full justify-end gap-3 mt-2">
              <Button
                ...
                onClick={onClose}
                disabled={isLoading}
                className='h-11 px-6 hover:text-slate-50 hover:bg-[#102A56]'
              >
                Cancel
              </Button>
              <Button
                ...
                className="h-11 bg-[#102A56] hover:bg-[#1d3a6d] text-white font-bold px-8 rounded-lg"
              >
