...
export const activateMyWithdrawalAccount = (accountId: string) =>
  apiRequest<WithdrawalAccount>(
    `/api/identity/me/withdrawal-accounts/${encodeURIComponent(accountId)}/activate`,
    {
      method: "PATCH",
      json: {
        isActive: true,
...
export const deleteMyWithdrawalAccount = (accountId: string) =>
  apiRequest<{ message: string }>(
    `/api/identity/me/withdrawal-accounts/${encodeURIComponent(accountId)}`,
    {
      method: "DELETE",
    },
  );
